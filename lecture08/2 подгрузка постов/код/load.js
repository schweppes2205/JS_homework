...
function init() {
    ...
    ////////////////question/////////////////
    // Подскажите пожалуйста, для чего необходимо рекурсивно вызывать init?
    // динамическая прогрузка после первой генерации страницы дальше будет обрабатываться 
    // событием scroll. Или есть еще какой-то сценарий, когда scroll не будет
    // отрабывать?
    ////////////////question/////////////////
...
function insertPosts() {
    ...
    let postsMarkup = '';
    for (let i = 1; i < 4; i++) {
        // в случае рандома, тут просто бОльший выбор картинок получается.
        postsMarkup += getPostMarkup(Math.round(Math.random() * 1000));
    }
...
function getPostMarkup(counter) {
    ...
    */
    ...
    let num = currentTime.getDate() + counter;
